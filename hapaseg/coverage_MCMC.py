...
from .model_optimizers import PoissonRegression, CovLNP_NR, CovLNP_NR_prior, covLNP_ll, covLNP_ll_prior
...
class AllelicCluster:
	def __init__(self, r, C, mu_0, beta_0, bin_width=1):
		...
		self.cache_lsigma_ptr = sp.dok_matrix(sz, dtype = np.int64); self.cache_lsigma = []
	...
	def get_ll(self):
		...
		for st, en in bdy:
			...
			if (ptr := self.cache_LL_ptr[st, en]) != 0:
				...
				ll += self.ll_segment([st, en], self.mu_i_arr[st], self.lsigma_i_arr[st])
		return ll
	...
	def lnp_optimizer(self, ind, ret_hess = False):
		if self.cache_mu_ptr[ind[0], ind[1]] != 0:
			...
			if ret_hess:
				return mu, lsigma, self.cache_hess[self.cache_hess_ptr[ind[0], ind[1]]]
			else:
				return mu, lsigma
		...
		self.cache_lsigma.append(res[1]); self.cache_lsigma_ptr[ind[0], ind[1]] = len(self.cache_lsigma) - 1
		self.cache_hess.append(res[2]); self.cache_hess_ptr[ind[0], ind[1]] = len(self.cache_hess) - 1
	...
	def _find_difs(self, ind):
		...
		windows = np.logspace(1, 3, 5).astype(int)
		windows = windows[2*windows < (ind[1] - ind[0])]
		difs = []
		for window in windows:
			difs.append(self._change_kernel(ind, residuals, window))
		difs_idxs = set().union(*difs)
		difs_idxs = np.r_[list(difs_idxs)]
		# if there are no that pass the threshold return them all (should not happen?)
		if len(difs_idxs) == 0:
			print('no significant bins', flush=True)
			return list(np.r_[ind[0] + 2:ind[1] - 1])

		# add on first and last windows edges since these are not considered by the kernel
	...
	def _neighborhood_sampling(self, ind, lls, split_indices, mus, lsigmas, Hs):
		...
		lls_add, mus_add, lsigmas_add, Hs_add = self._calculate_splits(ind, extra_samples)
		lls.extend(lls_add)
		mus.extend(mus_add)
	...
	def _detailed_sampling(self, ind, lls, split_indices, mus, lsigmas, Hs):
		...
		for ix in sig_idx:
			...
			while r_samples < max_samples:
				if ix_l < ind[0] + 2:
					break
				ll_add, mu_add, lsigma_add, Hs_add = self._calculate_splits(ind, [ix_l])
			...
			while l_samples < max_samples:
				if ix_r >= ind[1] -1:
					break
				ll_add, mu_add, lsigma_add, Hs_add = self._calculate_splits(ind, [ix_r])
	...
	def _get_split_liks(self, ind, debug=False):
		...
		# otherwise we deploy our change kernel to limit our search space
			...
			lls, mus, lsigmas, Hs = self._calculate_splits(ind, split_indices)
			lls, split_indices, mus, lsigmas, Hs = self._detailed_sampling(ind, lls, split_indices, mus, lsigmas, Hs)
	...
	def _log_ML_join(self, ind, ret_opt_params=False):
		...
		return mu_share, lsigma_share, self._get_log_ML_gaussint_join(H_share) + ll_join
	...
	def split(self, debug):
		...
		split_indices, log_split_MLs, all_mus, all_lsigmas = self._get_split_liks(ind, debug=debug)
