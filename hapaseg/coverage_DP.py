...
class Coverage_DP:
    def __init__(
        self, segmentation_draws, beta, cov_df, bin_width=1
    ):  # set to one for exomes, binsize for genomes
    ...
    def run_dp(
        self, num_seg_samples=-1, num_dp_samples=10, sample_idx=None
    ):  # option to pass sample number to scatter DP jobs
        ...
        if sample_idx is not None:
            ...
            if sample_idx > self.segmentation_draws.shape[1]:
                raise ValueError(
                    "invalid input segmentation sample number", self.sample_idx
                )
        ...
        for samp in range(self.num_seg_samples):
            ...
            self.cov_df["segment_ID"] = self.segmentation_draws[:, samp].astype(
                int
            )

            DP_runner = Run_Cov_DP(
                self.cov_df.copy(),
                self.beta,
                self.bin_exposure,
                prior_run,
    ...
    def visualize_DP_run(self, run_idx, save_path):
        ...
        for c in cov_dp.cluster_dict:
            for seg in cov_dp.cluster_dict[c]:
                resids = np.exp(
                    np.log(cov_dp.segment_r_list[seg])
                    - (
                        cov_dp.segment_C_list[seg] @ cov_dp.beta
...
class Run_Cov_DP:
    def __init__(
        self, cov_df, beta, bin_exposure, prior_run=None, count_prior_sum=None
    ):
        self.cov_df = cov_df
        self.seg_id_col = self.cov_df.columns.get_loc("segment_ID")
        self.beta = beta
        self.bin_exposure = bin_exposure
        self.covar_cols = sorted(
            self.cov_df.columns[
                self.cov_df.columns.str.contains("^C_.*z$|^C_log_len$")
            ]
        )
    ...
    def _init_segments(self):
        ...
        self.segment_idxs = (
            set(range(self.num_segments)) - self.blacklisted_segments
        )

        # set dp scale factor such that the average segment count is set to one
        self.dp_count_scale_factor = self.segment_sizes[
            self.segment_idxs
        ].mean()
        ...
        if query in self.cluster_prior_ml_cache:
            ...
            if new_segIDs is not None:
                ...
                C_new = np.concatenate(
                    [self.segment_C_list[s] for s in new_segIDs]
                )
                r = np.r_[
                    np.hstack([self.prior_r_list[i] for i in cluster_set]),
                    r_new,
                ]
                C = np.r_[
                    np.concatenate([self.prior_C_list[i] for i in cluster_set]),
                    C_new,
                ]
            else:
    ...
    def assign_greylist(self):
        ...
        for segID in self.greylist_segments:
            ...
            ML_BC = np.array(
                [
                    self._ML_cluster(self.cluster_dict[k].union([segID]))
        ...
        self.cluster_counts = sc.SortedDict(
            {
                c: self.segment_sizes[self.cluster_dict[c]].sum()
    ...
    def initial_prior_assignment(self, count_prior):
        for segID in range(self.num_segments):
            ...
            BC = np.r_[
                [
                    self._ML_cluster_prior(self.prior_clusters[c], [segID])
            ...
            self.cluster_MLs[choice] = self._ML_cluster(
                self.cluster_dict[choice]
            )
    ...
    def DP_move_cluster_prior(self, cur_cluster, prev_clusters):
        ...
        cluster_vals = (
            np.r_[prev_clusters, cluster_vals] / self.dp_count_scale_factor
        )
        N = cluster_vals.sum()
    ...
    def run(self, n_iter, sample_num=0):
        ...
        if self.prior_clusters:
            self.clust_prior_ML = sc.SortedDict(
                {
                    k: self._ML_cluster_prior(self.prior_clusters[k])
            ...
            count_prior = (
                np.r_[
                    [
                        v
                        for (k, v) in self.count_prior_sum.items()
                        if k in self.prior_clusters.keys()
                    ]
                ]
                / sample_num
            )
        ...
        while len(self.bins_to_clusters) < n_iter:
            ...
            if not n_it % 250 and self.prior_clusters is None:
                print(
                    "n clusters: {}".format(len(self.cluster_dict)), flush=True
                )
            ...
            if not n_it % 100:
                ...
                if (
                    ...
                    for ID in self.cluster_counts.keys():
                        self.cluster_LLs[ID] = self._LL_cluster(
                            self.cluster_dict[ID]
                        )
            ...
            if np.random.rand() < 0.5:
                if (
                    ...
                    and len(self.unassigned_segs) / len(white_segments) < 0.1
                ...
                # if cluster is empty without S ML is also 0
                    ...
                    ML_A = self._ML_cluster(
                        self.cluster_dict[clustID].difference([segID])
                    )
                ...
                if self.prior_clusters:
                    ...
                    prior_diff = (
                        self.prior_clusters.keys() - self.cluster_counts.keys()
                    )
                    ...
                    prior_null = (
                        self.cluster_counts.keys() - self.prior_clusters.keys()
                    )
                    ...
                    prior_idx = np.r_[
                        np.r_[
                            [self.prior_clusters.index(x) for x in prior_diff]
                        ],
                        np.r_[
                            [
                                self.prior_clusters.index(x)
                                if x in self.prior_clusters
                                else -1
                                for x in (
                                    prior_com
                                    | prior_null
                    ...
                    prior_MLs = np.r_[
                        [
                            self._ML_cluster_prior(
                                self.prior_clusters[
                                    self.prior_clusters.keys()[idx]
                                ],
                                [segID],
                            )
                            if idx != -1
                        ...
                        + np.r_[
                            [
                                ...
                                if idx != -1
                                else -self._ML_cluster([segID])
                                for idx in prior_idx
                            ]
                        ]
                    )
                    clust_prior_p = np.maximum(
                        np.exp(prior_MLs - prior_MLs.max())
                        / np.exp(prior_MLs - prior_MLs.max()).sum(),
                        1e-300,
                    )
                ...
                log_dp_count_prior = self.DP_move_segment_prior(
                    segID,
                    [
                        count_prior[self.prior_clusters.index(x)]
                        for x in prior_diff
                    ],
                )
                        ...
                        if len(self.cluster_dict[clustID]) == 1:
                            if old_clust:
                                ...
                                self.cluster_counts[choice] = (
                                    self.cluster_counts[clustID]
                                )
                                self.cluster_dict[choice] = self.cluster_dict[
                                    clustID
                                ].copy()
                                self.cluster_assignments[segID] = choice
                                self.cluster_MLs[choice] = self.cluster_MLs[
                                    clustID
                                ]
                                ...
                                if all_assigned:
                                    self.cluster_LLs[choice] = self._LL_cluster[
                                        clustID
                                    ].copy()
                            ...
                            self.cluster_counts[clustID] -= self.segment_sizes[
                                segID
                            ]
                            ...
                            if all_assigned:
                                self.cluster_LLs[clustID] = self._LL_cluster(
                                    self.cluster_dict[clustID]
                                )
                    else:
            ...
            # pick cluster to merge
                ...
                ML_join = [
                    self._ML_cluster(
                        self.cluster_dict[i].union(clust_pick_segs)
                    )
                    if i != clust_pick
                    ...
                    prior_MLs = np.r_[
                        ...
                        + np.r_[
                            [
                                ...
                                if idx != -1
                                else -self.cluster_MLs[clust_pick]
                                for idx in prior_idx
                            ]
                        ]
                    )
                ...
                log_dp_count_prior = self.DP_move_cluster_prior(
                    clust_pick,
                    [
                        count_prior[self.prior_clusters.index(x)]
                        for x in prior_diff
                ...
                # last = brand new, -1, -2, -3, ... = -(prior clust index) - 1
                choice = np.r_[
                    -np.r_[prior_diff] - 1, self.cluster_counts.keys()
                ][choice_idx]
                choice = int(choice)
                ...
                if choice != clust_pick:
                    if choice < 0:
                        ...
                        choice = -(choice + 1)
                        # rename clustID to choice
                        self.cluster_counts[choice] = self.cluster_counts[
                            clust_pick
                        ]
                        self.cluster_dict[choice] = self.cluster_dict[
                            clust_pick
                        ...
                        if all_assigned:
                            self.cluster_LLs[choice] = self.cluster_LLs[
                                clust_pick
                            ].copy()
                        ...
                        self.cluster_counts[merged_ID] += self.segment_sizes[
                            vacating_segs
                        ].sum()
                        self.cluster_dict[merged_ID] = self.cluster_dict[
                            merged_ID
                        ].union(vacating_segs)
                        self.cluster_MLs[merged_ID] = ML_join[
                            list(self.cluster_counts.keys()).index(choice)
                        ]

                        if all_assigned:
                            self.cluster_LLs[merged_ID] = self._LL_cluster(
            ...
            if burned_in and n_it - n_it_last > len(white_segments) / (
                len(self.cluster_counts) * 2
            ):
