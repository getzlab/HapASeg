...
class AllelicCluster:
	...
	def get_ll(self):
		...
		for st, en in bdy:
			...
			if (ptr := self.cache_LL_ptr[st, en]) != 0:
				...
				ll += self.ll_segment([st, en], self.mu_i_arr[st], self.lepsi_i_arr[st])
		return ll
	...
	def lnp_init(self):
		# run plain Poisson regression without fitting beta to get mu prior
		prior = PoissonRegression(
		  self.r,
		  np.zeros([len(self.r), 0]), # don't fit beta
		  np.ones_like(self.r),
		  log_exposure = np.log(self.bin_exposure),
		  log_offset = self.C@self.beta # beta included as offset
		)
		prior.fit()

		# now fit LNP
		lnp = CovLNP_NR_prior(self.r, self.beta, self.C, exposure = np.log(self.bin_exposure), init_prior=True, lamda = self.lamda, mu_prior = prior.mu[0, 0], alpha_prior = 1, beta_prior = 5e-2)
		return lnp.fit()
	...
	def lnp_optimizer(self, ind, ret_hess = False):
		...
		self.cache_lepsi.append(res[1]); self.cache_lepsi_ptr[ind[0], ind[1]] = len(self.cache_lepsi) - 1
		...
		for ix in split_indices:
			if ix < 0:
				...
				if (ptr := self.cache_LL_ptr[ind[0], ix]) != 0:
					...
					ll_l = self.ll_segment([ind[0], ix], mu_l, lepsi_l)
				...
				if (ptr := self.cache_LL_ptr[ix, ind[1]]) != 0:
					...
					ll_r = self.ll_segment([ix, ind[1]], mu_r, lepsi_r)
	...
	def _log_ML_join(self, ind, ret_opt_params=False):
		...
		if (ptr := self.cache_LL_ptr[ind[0], ind[1]]) != 0:
			...
			ll_join = self.ll_segment(ind, mu_share, lepsi_share)
