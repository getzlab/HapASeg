...
def generate_acdp_df(
    ...
    if cdp_object_path is not None:
        ...
        for run_idx, cdp_file in enumerate(cdp_file_list):
            with open(cdp_file, "rb") as cdp:
                ...
                print(
                    "file {} : len dp runs: {}".format(
                        cdp_file, len(dp_pickle.DP_runs)
                    ),
                    flush=True,
                )
        ...
        for run in runs_considered:
            ...
            dp_assignments = dp_assignments[
                ~cov_df_run.segment_ID.isin(small_segs)
            ]
        ...
        raise ValueError(
            "must pass in either a cdp object path or a txt file containing a list of object paths"
        )
    ...
    for draw_num, cov_df_run, dp_assignments in dp_run_data:
        ...
        covar_cols = sorted(
            a_cov_seg_df.columns[
                a_cov_seg_df.columns.str.contains("(?:^C_.*z$|C_log_len)")
            ]
        )
        ...
        if cov_df_path is None:
            ...
            for seg in range(len(segs_to_clusts)):
                a_cov_seg_df.loc[
                    a_cov_seg_df["segment_ID"] == seg, "cov_DP_cluster"
                ] = segs_to_clusts[seg]
        ...
        for adp, cdp in tqdm.tqdm(
            a_cov_seg_df.groupby(["allelic_cluster", "cov_DP_cluster"]).indices
        ):
            acdp_clust = a_cov_seg_df.loc[
                (a_cov_seg_df.cov_DP_cluster == cdp)
                & (a_cov_seg_df.allelic_cluster == adp)
            ]
            if len(acdp_clust) < 5:
                acdp_clust = a_cov_seg_df.loc[
                    a_cov_seg_df.cov_DP_cluster == cdp
            ...
            lnp = CovLNP_NR_prior(
                ...
                beta,
                C,
                exposure=np.log(bin_width),
                alpha_prior=alpha_prior,
                beta_prior=beta_prior,
                mu_prior=np.log(r).mean(),
                lamda=1e-10,
                init_prior=False,
            )
            ...
            try:
                ...
                if filter_mask.sum() == 0:
                    ...
                    r = r[filter_mask]
                    C = C[filter_mask]
                    a_cov_seg_df = a_cov_seg_df.drop(
                        acdp_clust.index[~filter_mask]
                    )
                    lnp = CovLNP_NR_prior(
                        r[:, None],
                        beta,
...
class AllelicCoverage_DP_runner:
    ...
    def _init_segments(self):
        ...
        for ID, (name, grouped) in enumerate(
            ...
            if group_len > 10:
                major, minor = (
                    grouped["maj_count"].sum(),
                    grouped["min_count"].sum(),
                )
            else:
                ...
                if ADP_clust in fallback_counts:
                    ...
                    filt = self.cov_df.loc[
                        self.cov_df.allelic_cluster == ADP_clust
                    ]
                    major, minor = (
                        filt["maj_count"].sum(),
                        filt["min_count"].sum(),
            ...
            r = np.array(
                s.poisson.rvs(
                    np.exp(
                        s.norm.rvs(
                            norm_samples[:, 0], np.exp(norm_samples[:, 1])
                        )
                    )
                    * s.beta.rvs(a + 1, b + 1, size=group_len)
                )
            )
        ...
        boundary_intercept = (
            2.5 * median_std - (median_mean * slope + intercept)
        ) + intercept
        self.greylist_segments.update(
            np.r_[: self.num_segments][
                (
                    means * slope + boundary_intercept
                    < np.sqrt(self.segment_V_list)
                )
            ]
        )
    ...
    def _generate_base_unclustered_segs(self, opt_idx=None):
        ...
        segs_df = segs_df.rename(
            columns={"chr": "Chromosome", "start": "Start.bp", "end": "End.bp"}
        )
    ...
    def generate_unclustered_segs(self, opt_idx=None, absolute_format=True):
        ...
        if absolute_format:
            ...
            segs_df = segs_df.drop(
                ["emp.sigma.major", "emp.sigma.minor"], axis=1
            )
            segs_df = segs_df.rename(
                {
                    "post.sigma.major": "sigma.major",
                    "post.sigma.minor": "sigma.minor",
                },
                axis=1,
            )

            pd.options.mode.chained_assignment = None
            segs_df["f"] = segs_df["min_count"] / (
                segs_df["min_count"] + segs_df["maj_count"]
            )
            ...
            sf = (
                segs_df["length"] @ segs_df["tau"] / segs_df["length"].sum() / 2
            )
            segs_df.loc[
                :,
                [
            ...
            segs_df = segs_df.drop(
                ["post.sigma.major", "post.sigma.minor"], axis=1
            )
            segs_df = segs_df.rename(
                {
                    "emp.sigma.major": "sigma.major",
                    "emp.sigma.minor": "sigma.minor",
                },
                axis=1,
            )
    ...
    def generate_unclustered_plot(
        self, savepath, ref_idx, cytoband_file, opt_idx=None
    ):
        segs_df = self._generate_base_unclustered_segs(opt_idx=opt_idx)
        segs_df["start_g"] = seq.chrpos2gpos(
            segs_df["Chromosome"], segs_df["Start.bp"], ref=ref_idx
        )
        segs_df["end_g"] = seq.chrpos2gpos(
            segs_df["Chromosome"], segs_df["End.bp"], ref=ref_idx
        )
        ...
        for _, row in segs_df.iterrows():
            ax.add_patch(
                patches.Rectangle(
                    ...
                    row.end_g - row.start_g,
                    2 * 1.95 * row["emp.sigma.major"],
                    fill=True,
                    facecolor="r",
                    alpha=0.1,
                    edgecolor="none",
            ...
            ax.plot(
                [row.start_g, row.end_g],
                [row["mu.major"], row["mu.major"]],
                alpha=1,
                color="r",
                linewidth=0.4,
            )
        ...
        plt.ylim(
            [
                min(
                    segs_df["mu.minor"] - 10 - 1.95 * segs_df["emp.sigma.minor"]
                ),
                max(
                    segs_df["mu.major"] + 10 + 1.95 * segs_df["emp.sigma.major"]
                ),
            ]
        )
    ...
    def run_seperated(self, n_iter):
        ...
        single_allele_df = self.cov_df.loc[
            (self.cov_df.dp_draw == self.default_draw)
            & (self.cov_df.allele == -1)
        ]
        ## aggregate het counts across allelic DP segments
        snp_segments_df = single_allele_df.groupby("segment_ID").apply(
            lambda x: pd.Series(
                {
                    "weight": len(x),
                    "min_count": x["min_count"].sum(),
                    "maj_count": x["maj_count"].sum(),
                    "f": x["min_count"].sum()
                    / (x["min_count"].sum() + x["maj_count"].sum()),
                }
            )
        )
        ...
        beta_dict = {
            c: s.beta(r.min_count + 1, r.maj_count + 1)
            for c, r in snp_segments_df.iterrows()
        }
        ...
        alpha = np.linspace(0.0, 1.0, 10000)
        ...
        purity_liks = np.exp(
            opt_states[find_peaks(opt_states, prominence=10, distance=10)[0]]
            - opt_states.max()
        )
        if (purity_liks > 0.01).sum() > 1:
            print(
                f"WARNING: multiple purities possible, using optimal purity {opt_purity}",
                flush=True,
            )
        else:
        ...
        try:
            kneedle = KneeLocator(
                sorted_cum_weights, sorted_liks[:, 0], S=2.0, online=True
            )
            clonal_lik_threshold = sorted_liks[
                np.where(sorted_cum_weights == kneedle.elbow)[0][0]
            ][0]
        except:
            print(
                "Failed elbow fitting, falling back on using half of the segments as clonal"
            )
            n_segs = len(sorted_liks)
            # set threshold to the median lik, using the left median in the case of ties
            clonal_lik_threshold = sorted_liks[
                int(n_segs / 2) if n_segs % 2 else int(n_segs / 2 - 1)
            ][0]

        clonal_segIDs = np.array(list(beta_dict.keys()))[
            purity_res.max(1)[opt_purity_idx] > clonal_lik_threshold
        ]
        clonal_segs = self.cov_df.loc[
            self.cov_df.segment_ID.isin(clonal_segIDs)
        ].acdp_segID.unique()
        ...
        clonal_datapoints = [
            np.hstack(
                [
                    acdp_clonal_first.segment_r_list[s]
                    for s in acdp_clonal_first.cluster_dict[c]
                ]
            )
            for c in acdp_clonal_first.cluster_dict
        ]
        ...
        consts = np.array(
            [ks * ((1 - opt_purity) + a * opt_purity) for a in range(0, 6)]
        )
        sigmas = np.array(
            [seg_datapoints.std() for seg_datapoints in clonal_datapoints]
        )
                ...
                [
                    [
                        (
                            ...
                            - (
                                (clonal_datapoints[i] - consts[ac][k] ** 2)
                                / (2 * sigmas[i] ** 2)
                            )
                        ).sum(0)
        ...
        comb_liks = np.exp(
            opt_comb[find_peaks(opt_comb, prominence=10, distance=10)[0]]
            - opt_comb.max()
        )
        if (comb_liks > 0.01).sum() > 1:
            print(
                f"WARNING: multiple k values possible, using optimal k= {opt_k}",
                flush=True,
            )
        else:
        ...
        opt_covcomb = covcomb_res[
            :, :, covcomb_res.max(1).sum(0).argmax()
        ]  # na liks for optimal k
        ...
        for i, c in enumerate(acdp_clonal_first.cluster_dict):
            ...
            null_lik = (
                np.log(
                    1
                    / (clonal_datapoints[i].max() - clonal_datapoints[i].min())
                )
                * clonal_sizes[i]
            )
        ...
        target_clonal_datapoints = [
            (
                c,
                np.hstack(
                    [
                        self.segment_r_list[s]
                        for s in acdp_clonal_first.cluster_dict[c]
                    ]
                ),
            )
            for c in clonal_clusters
        ]
        target_normals = {
            tc_data[0]: s.norm(tc_data[1].mean(), tc_data[1].std())
            for tc_data in target_clonal_datapoints
        }
        ...
        subclonal_single_segs = np.array(list(beta_dict.keys()))[
            purity_res.max(1)[opt_purity_idx] <= clonal_lik_threshold
        ]
        subclonal_segs = self.cov_df.loc[
            self.cov_df.segment_ID.isin(subclonal_single_segs)
        ].acdp_segID.unique()
        subclonal_segs_data = [
            (s, self.segment_r_list[s]) for s in subclonal_segs
        ]
        ...
        for seg, seg_datapoints in subclonal_segs_data:
            ...
            assgn = list(seg_res.keys())[
                np.array(list(seg_res.values())).argmax()
            ]
            if assgn == -1:
        ...
        clonal_segs_final = np.r_[
            np.hstack(
                [
                    list(acdp_clonal_first.cluster_dict[c])
                    for c in clonal_clusters
                ]
            ),
            clonal_classified,
        ].astype(int)
        subclonal_segs_final = np.array(
            list(set(range(self.num_segments)) - set(clonal_segs_final))
        ).astype(int)
        ...
        unique_cluster_labels = {
            ...
            for c, v in zip(
                *np.unique(
                    np.r_[
                        list(acdp_clonal.cluster_dict.keys()),
                        list(acdp_subclonal.cluster_dict.keys()),
                    ],
                    return_index=True,
                )
            )
        ...
        combined_cluster_dict = {
            **{
                unique_cluster_labels[c]: v
                for c, v in acdp_clonal.cluster_dict.items()
            },
            **{
                unique_cluster_labels[c]: v
                for c, v in acdp_subclonal.cluster_dict.items()
            },
        }
        ...
        acdp_combined.cluster_counts = sc.SortedDict(
            {
                c: acdp_combined.segment_counts[
                    list(acdp_combined.cluster_dict[c])
                ].sum()
                for c in acdp_combined.cluster_dict
            }
        )
        acdp_combined.cluster_sums = sc.SortedDict(
            {
                c: v.sum()
                for c, v in [
                    (
                        ...
                        np.hstack(
                            [
                                acdp_combined.segment_r_list[s]
                                for s in acdp_combined.cluster_dict[c]
                            ]
                        ),
                    )
                    for c in acdp_combined.cluster_dict
        ...
        acdp_combined.cluster_ssd = sc.SortedDict(
            {
                c: v.var() * len(v)
                for c, v in [
                    (
        ...
        for c in acdp_combined.cluster_dict:
            acdp_combined.cluster_assignments[
                list(acdp_combined.cluster_dict[c])
            ] = c
        acdp_combined.cluster_MLs = sc.SortedDict(
            {
                c: acdp_combined._ML_cluster_from_list(
                    acdp_combined.cluster_dict[c]
                )
                for c in acdp_combined.cluster_dict
            }
        )
...
class AllelicCoverage_DP:
    ...
    def ML_normalgamma_params(self, n, x_mean, ssd):
        ...
        beta_n = (
            self.beta_0
            + 0.5 * ssd
            + self.kappa_0 * n * (x_mean - mu0) ** 2 / 2 * (self.kappa_0 + n)
        )
    ...
    def ML_normalgamma(self, n, x_mean, ssd):
        mu_n, kappa_n, alpha_n, beta_n = self.ML_normalgamma_params(
            n, x_mean, ssd
        )
        return (
            ss.loggamma(alpha_n)
            - self.loggamma_alpha_0
    ...
    def _ssd_cluster_add_one(self, clusterID, segID):
        ...
        mu_m = sum_m / m
        ssd = (
            self.cluster_ssd[clusterID]
            + self.segment_ssd[segID]
            + m * n / (m + n) * (mu_m - mu_n) ** 2
        )
        return mn, mu_mn, ssd
    ...
    def _ssd_cluster_merge(self, clust_A, clust_B):
        ...
        ssd = (
            self.cluster_ssd[clust_A]
            + self.cluster_ssd[clust_B]
            + m * n / (m + n) * (mu_m - mu_n) ** 2
        )
        return mn, mu_mn, ssd
    ...
    def save_ML_total(self):
        ...
        DP_tot = (
            num_clusts * np.log(self.alpha)
            + sum([ss.gammaln(na) for na in self.cluster_counts.values()])
            - ss.gammaln(self.alpha + N)
            + ss.gammaln(self.alpha)
        )
        ...
        for i, nc in enumerate(cluster_vals):
            if i != cur_index:
                ...
                prior_results[i] = (
                    ss.gammaln(M)
                    + np.log(self.alpha)
                    + ss.gammaln(N + self.alpha - M)
                    - ss.gammaln(N + self.alpha)
                )
    ...
    def DP_tuple_split_prior(self, seg_id):
        ...
        for i, nc in enumerate(cluster_vals):
            prior_results[i] = (
                ss.loggamma(seg_size + nc)
                + loggamma_N_alpha_seg
                - (ss.loggamma(nc) + loggamma_N_alpha)
            )
    ...
    def assign_greylist_MLs(self):
        ...
        greylist_added_dict = sc.SortedDict(
            {k: v.copy() for k, v in self.cluster_dict.items()}
        )
        ...
        for segID in self.greylist_segments:
            ...
            ML_BC = np.array(
                [
                    self._ML_cluster_add_one(k, segID)
            ...
            choice_p = (
                np.exp(ML_rat + log_count_prior - MLs_max)
                / np.exp(ML_rat + log_count_prior - MLs_max).sum()
            )
    ...
    def assign_greylist(self):
        ...
        norms = {
            c: s.norm(seg.mean(), seg.std())
            for c, seg in [
                (
                    c,
                    np.hstack(
                        [self.segment_r_list[i] for i in self.cluster_dict[c]]
                    ),
                )
                for c in clusters
            ]
        ...
        for segID in self.greylist_segments:
            ...
            if log_liks.max() > null_lik:
                ...
                self.cluster_MLs[clusterID] = self._ML_cluster_add_one(
                    clusterID, segID
                )
                self.cluster_ssd[clusterID] = self._ssd_cluster_add_one(
                    clusterID, segID
                )[2]
                continue
            else:
                if len(r) > 25:
                    print(
                        f"segment {segID} given new cluster {self.next_cluster_index}"
                    )
                    # if we have a farily large segment we let it form its own cluster
                    self.cluster_dict[self.next_cluster_index] = sc.SortedSet(
                        [segID]
                    )
                    ...
                    self.cluster_MLs[self.next_cluster_index] = (
                        self._ML_cluster_direct(
                            len(r), r.mean(), len(r) * r.var()
                        )
                    )
                    self.next_cluster_index += 1
                else:
    ...
    def run(self, n_iter):
        ...
        while len(self.bins_to_clusters) < n_iter:
            ...
            if not n_it % 250:
                print(
                    "{} iterations; num_clusters: {}; ML:{}".format(
                        n_it,
                        len(self.cluster_dict.keys()),
                        self.MLDP_total_history[-1],
                    ),
                    flush=True,
                )
            ...
            if not n_it % 100:
                ...
                if (
                    not all_assigned
                    and (self.cluster_assignments[white_segments] > -1).all()
                ):
                    all_assigned = True
                    n_it_last = n_it
                ...
                if (
                    not burned_in
                    and all_assigned
                    and n_it - n_it_last > max(2000, len(self.segments))
                ):
                    if (
                        np.diff(np.r_[self.MLDP_total_history[-2000:]]).mean()
                        <= 0
                    ):
            ...
            if np.random.rand() < 0.5:
                if (
                    len(self.unassigned_segs) > 0
                    and len(self.unassigned_segs) / len(self.segments) < 0.1
                ...
                if clustID > -1:
                    ML_rat_BC[
                        list(self.cluster_counts.keys()).index(clustID)
                    ] = 0
                ...
                if np.isnan(choice_p.sum()):
                    print(
                        "skipping iteration {} due to nan. picked segment {}".format(
                            n_it, segID
                        ),
                        flush=True,
                    )
                ...
                choice_idx = np.random.choice(
                    np.r_[0 : len(ML_rat)], p=choice_p
                )

                # last = brand new, -1, -2, -3, ... = -(prior clust index) - 1
                choice = np.r_[
                    self.cluster_counts.keys(), self.next_cluster_index
                ][choice_idx]
                choice = int(choice)
                ...
                if choice == self.next_cluster_index:
                    if clustID > -1:
                        ...
                        if len(self.cluster_dict[clustID]) == 1:
                            ...
                            self.cluster_ssd[clustID] = (
                                self._ssd_cluster_remove_one(clustID, segID)[2]
                            )
                            self.cluster_counts[clustID] -= self.segment_counts[
                                segID
                            ]
                            self.cluster_dict[clustID].discard(segID)
                            self.cluster_MLs[clustID] = ML_A
                            self.cluster_sums[clustID] -= self.segment_sums[
                                segID
                            ]
                    else:
                    ...
                    self.cluster_assignments[segID] = choice
                    self.cluster_ssd[choice] = self._ssd_cluster_add_one(
                        choice, segID
                    )[2]
                    self.cluster_counts[choice] += self.segment_counts[segID]
                    self.cluster_dict[choice].add(segID)
                    self.cluster_MLs[choice] = ML_BC[
                        list(self.cluster_counts.keys()).index(choice)
                    ]
                    self.cluster_sums[choice] += self.segment_sums[segID]
            ...
            # pick cluster to merge or split
                ...
                clust_pick_segs = np.r_[self.cluster_dict[clust_pick]].astype(
                    int
                )
                ...
                if np.random.rand() < 0.5:
                    ...
                    seg_means = (
                        self.segment_sums[clust_pick_segs]
                        / self.segment_counts[clust_pick_segs]
                    )
                    ...
                    ssd_A = sorted_ssds[0]
                    ssd_B = self._ssd_cluster_remove_one(
                        clust_pick, sorted_segs[0]
                    )[2]
                    ...
                    choice_p = (
                        np.exp(tot_list - tot_max)
                        / np.exp(tot_list - tot_max).sum()
                    )
                    ...
                    if np.random.rand() < split_prob:
                        ...
                        self.cluster_counts[clust_pick] = sum(
                            self.segment_counts[A_list]
                        )
                        self.cluster_dict[clust_pick] = sc.SortedSet(A_list)
                        self.cluster_MLs[clust_pick] = (
                            self._ML_cluster_from_list(A_list)
                        )
                        self.cluster_sums[clust_pick] = self.segment_sums[
                            A_list
                        ].sum()
                        self.cluster_ssd[clust_pick] = ssd_A
                        # create new cluster with next available index and add segments from list B
                        self.cluster_assignments[B_list] = (
                            self.next_cluster_index
                        )
                        self.cluster_counts[self.next_cluster_index] = sum(
                            self.segment_counts[B_list]
                        )
                        self.cluster_dict[self.next_cluster_index] = (
                            sc.SortedSet(B_list)
                        )
                        self.cluster_MLs[self.next_cluster_index] = (
                            self._ML_cluster_from_list(B_list)
                        )
                        self.cluster_sums[self.next_cluster_index] = (
                            self.segment_sums[B_list].sum()
                        )
                ...
                # otherwise we'll propose a merge
                    ...
                    ML_join = [
                        self._ML_cluster_merge(i, clust_pick)
                        if i != clust_pick
                        else self.cluster_MLs[i]
                    ...
                    ML_split = (
                        np.array(self.cluster_MLs.values())
                        + self.cluster_MLs[clust_pick]
                    )
                    ML_split[self.cluster_MLs.keys().index(clust_pick)] = (
                        self.cluster_MLs[clust_pick]
                    )
                    ...
                    choice_p = (
                        np.exp(ML_rat + count_prior - MLs_max)
                        / np.exp(ML_rat + count_prior - MLs_max).sum()
                    )

                    if np.isnan(choice_p.sum()):
                        print(
                            "skipping iteration {} due to nan".format(n_it),
                            flush=True,
                        )
                        # print(ML_rat)
                        # print(count_prior)
                    ...
                    if choice != clust_pick:
                        ...
                        larger_cluster = np.argmax(
                            [
                                self.cluster_counts[clust_pick],
                                self.cluster_counts[choice],
                            ]
                        )
                        ...
                        self.cluster_ssd[merged_ID] = self._ssd_cluster_merge(
                            merged_ID, vacatingID
                        )[2]
                        self.cluster_counts[merged_ID] += self.segment_counts[
                            vacating_segs
                        ].sum()
                        self.cluster_dict[merged_ID] = self.cluster_dict[
                            merged_ID
                        ].union(vacating_segs)
                        self.cluster_MLs[merged_ID] = ML_join[choice_idx]
                        self.cluster_sums[merged_ID] += self.cluster_sums[
                            vacatingID
                        ]
    ...
    def _get_color_palette(self, num_colors):
        ...
        return np.r_[
            base_colors,
            extra_colors if extra_colors.size > 0 else np.empty([0, 3]),
        ]
    ...
    def _get_cluster_colors(self):
        ...
        si = np.argsort(
            np.argsort(
                [
                    -self._get_clust_terr(self.cluster_dict[c], full_df)
    ...
    def _get_cdp_colors(self, cdp_draw=0):
        chosen_draw = self.cov_df.loc[
            (self.cov_df.dp_draw == cdp_draw) & (self.cov_df.allele == 1)
        ]
        ...
        return {
            ...
            for i, c in enumerate(
                chosen_draw.cov_DP_cluster.value_counts().index
            )
        }
    ...
    def _get_adp_colors(self):
        ...
        return {
            c: palette[i]
            for i, c in enumerate(
                chosen_draw.allelic_cluster.value_counts().index
            )
        }
    ...
    def _get_real_cov(self, df):
        covar_cols = sorted(
            df.columns[df.columns.str.contains("(?:^C_.*z$|C_log_len)")]
        )
    ...
    def precompute_cluster_params(self, cdp_draw=None):
        ...
        for i, c in enumerate(self.cluster_dict.keys()):
            ...
            clust_data[c] = {
                "mean": clust_r_arr.mean(),
                "std": clust_r_arr.std(),
                "terr": clust_terr,
                "datapoints": clust_r_arr,
                "mean_uncertainty": _mean_uncertainty(clust_r_arr),
            }
        clust_terrs = np.array(
            [clust_data[c]["terr"] for c in clust_data.keys()]
        )
    ...
    def visualize_ACDP(
        ...
        ADP_dict = {}
        for ADP, group in self.cov_df.loc[
            self.cov_df.dp_draw == self.default_draw
        ].groupby("allelic_cluster"):
        ...
        for i, c in enumerate(cluster_stats.keys()):
            ...
            for tup in self.cluster_dict[c]:
                ...
                if quad[3] == 0 or (
                    cdp_draw is not None and quad[3] == cdp_draw
                ):
                ...
                if not self.wgs:
                    ...
                    for intv in seg_intervals:
                        ...
                        tup_width = (
                            x.iloc[intv[1]].end_g - x.iloc[intv[0]].start_g
                        )
                        ...
                        ax_g.add_patch(
                            patches.Rectangle(
                                ...
                                edgecolor="b"
                                if tup_allele > 0
                                else "r",  # color edges according to allele
                                ...
                                if tup_allele > 0
                                    ...
                                    0,
                                    (5, 0, 0, 5),
                                ),  # color edges with alternating pattern according to allele
                            )
                        )
            ...
            if self.wgs:
                ...
                all_cluster_bins = pd.concat(
                    [
                        full_df[tup][1]
                        for tup in self.cluster_dict[c]
                        if full_df[tup][0][3] == cdp_draw_idx
                    ]
                )
                all_cluster_bins = all_cluster_bins.sort_index()
                # plot patches for each contiguous segment within a adp, allele tuple
                for label, bins in all_cluster_bins.groupby(
                    ["allelic_cluster", "allele"]
                ):
                    ...
                    for intv in seg_intervals:
                        ...
                        ax_g.add_patch(
                            patches.Rectangle(
                                ...
                                facecolor=np.r_[
                                    cluster_colors[i],
                                    0.5 if cdp_draw is None else 0.8,
                                ],
                                fill=True,
                                alpha=1,
                        ...
                        ax_g.add_patch(
                            patches.Rectangle(
                                ...
                                linewidth=lw_wgs_large
                                if tup_width > 1000000
                                else lw_wgs_small,
                                ls=(0, (0, 5, 5, 0))
                                if tup_allele > 0
                                else (
            ...
            cluster_stats[c]["real_data"] = (
                np.concatenate(cluster_real_data)
                if len(cluster_real_data) > 0
                else []
            )
        ...
        round_max_acov = 25 * int(
            np.ceil(
                max([d["mean"] + 4 * d["std"] for d in cluster_stats.values()])
                / 25
            )
        )
        ...
        if plot_hist:
            ...
            for i, c in enumerate(cluster_stats.keys()):
                ax_hist.hist(
                    cluster_stats[c]["datapoints"],
                    bins=np.r_[:round_max_acov:hist_bin_width],
                    alpha=0.5,
        ...
        last_chr_idx = (
            np.searchsorted(np.cumsum(ends.end), self.cov_df.end_g.max()) + 1
        )
    ...
    def visualize_ACDP_clusters(self, save_path):
        ...
        for c in self.cluster_dict:
            rs.append(
                (
                    np.array(
                        [
                            np.array(self.segment_r_list[i]).mean()
                            for i in self.cluster_dict[c]
                        ]
                    ).mean(),
                    c,
                )
            ...
            rmax = max(
                ...
                np.array(
                    [
                        np.array(self.segment_r_list[i]).max()
                        for i in self.cluster_dict[c]
                    ]
                ).max(),
            )
        ...
        for c in [t[1] for t in sorted(rs)]:
            c0 = counter
            vals = [
                np.array(self.segment_r_list[i]) for i in self.cluster_dict[c]
            ]

            for arr in vals:
                ax.scatter(
                    np.repeat(counter, len(arr)),
                    arr,
                    marker=".",
                    alpha=0.05,
                    s=4,
        ...
        for c in [t[1] for t in sorted(rs)]:
            ...
            ax.scatter(
                np.r_[counter : counter + len(vals)],
                vals,
                alpha=[
        ...
        for c in self.cluster_dict:
            for seg in self.cluster_dict[c]:
                self.cov_df.loc[  # type: ignore
                    ...
                    cluster_stats[c]["mean"],
                    cluster_stats[c]["std"],
                    cluster_stats[c]["mean_uncertainty"],
                )
    ...
    def create_allelic_segs_df(self, absolute_format=False):
        ...
        for i in range(num_bins):
            ...
            if (
                cur_chr != maj_chr
                or cur_maj_cluster != maj_cluster
                or cur_min_cluster != min_cluster
            ):
                collapsed.append(
                    (
                        maj_chr,
                ...
                _, _, _, min_cluster, min_mu, min_sigma, min_mu_unc = min_vals[
                    i
                ]
                last_i = i
            else:
        ...
        seg_df = pd.DataFrame(
            ...
            columns=[
                ...
                "mu.major",
                "sigma.major",
                "mu_uncertainty.major",
                "mu.minor",
                "sigma.minor",
                "mu_uncertainty.minor",
                "num_bins",
            ],
        ).astype(
            {
                ...
                "mu.major": float,
                "sigma.major": float,
                "mu_uncertainty.major": float,
                "mu.minor": float,
                "sigma.minor": float,
                "mu_uncertainty.minor": float,
            }
        )
        ...
        if absolute_format:
            ...
            seg_df = seg_df.drop(
                ["mu_uncertainty.major", "mu_uncertainty.minor"], axis=1
            )
            ...
            sf = (
                ...
                / seg_df.loc[nan_mask]["length"].sum()
                / 2
            )
            seg_df.loc[
                :,
            ...
            ].astype({"Chromosome": int, "Start.bp": int, "End.bp": int})
