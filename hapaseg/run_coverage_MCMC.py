...
class CoverageMCMCRunner:
    def __init__(self,
        ...
        if allelic_sample:
            ...
            self.allelic_sample = np.choice(num_samples)
    ...
    def run(self):
        ...
        cov_mcmc = NB_coverage_MCMC(self.num_draws, r, C, Pi, self.save_dir)
        cov_mcmc.run()
        filtered_cov_df.to_hdf(os.path.join(self.save_dir, 'coverage_results.h5'), key='cov_df')
    ...
    def load_SNPs(self, f_snps):
        ...
        SNPs["tidx"] = mut.map_mutations_to_targets(SNPs, self.cov_df, inplace=False)
        return SNPs
    ...
    def load_covariates(self):
        ...
        self.cov_df["C_len"] = self.cov_df["end"] - self.cov_df["start"] + 1

        # load repl timing
        F = pd.read_pickle(os.path.join(self.covariate_dir, "GSE137764_H1.hg19_liftover.pickle"))

        # map targets to RT intervals
        tidx = mut.map_mutations_to_targets(self.cov_df.rename(columns={"start": "pos"}), F, inplace=False)
        self.cov_df.loc[tidx.index, "C_RT"] = F.iloc[tidx, 3:].mean(1).values

        # z-transform
        self.cov_df["C_RT_z"] = (lambda x: (x - np.nanmean(x)) / np.nanstd(x))(np.log(self.cov_df["C_RT"] + 1e-20))

        # load GC content
        B = pd.read_pickle(os.path.join(self.covariate_dir, "GC.pickle"))
        self.cov_df.loc = self.cov_df.merge(B.rename(columns={"gc": "C_GC"}), left_on=["chr", "start", "end"],
                                right_on=["chr", "start", "end"], how="left")
        self.cov_df.loc["C_GC_z"] = (lambda x: (x - np.nanmean(x)) / np.nanstd(x))(np.log(self.cov_df.loc["C_GC"] + 1e-20))
    ...
    def assign_clusters(self):
        ...
        new_assgn = np.array([np.random.choice(np.r_[:clust_choice.shape[1]],
