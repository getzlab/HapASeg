...
class CoverageMCMCRunner:
    def __init__(
        ...
        if allelic_sample is not None:
            ...
            self.allelic_sample = np.argmax(
                self.allelic_clusters["likelihoods"]
            )
    ...
    def prepare_single_cluster(self):
        cluster_map, observations, covariates, filtered_cov_df = (
            self.make_regressors(self.aseg_cov_df)
        )
        ...
        full_mask = filter_segments(
            ...
            global_beta,
            all_mu,
            exposure=np.log(self.bin_width),
        ...
        return (
            cluster_map[full_mask],
            observations[full_mask],
            covariates[full_mask],
            all_mu,
            global_beta,
            filtered_cov_df.loc[full_mask],
            self.allelic_sample,
        )
    ...
    def load_coverage(self, coverage_csv):
        ...
        Cov["start_g"] = seq.chrpos2gpos(
            Cov["chr"], Cov["start"], ref=self.ref_fasta
        )
        Cov["end_g"] = seq.chrpos2gpos(
            Cov["chr"], Cov["end"], ref=self.ref_fasta
        )
        ...
        ):  # suppress erroneous SettingWithCopyWarning
            Cov["fragcorr"] = np.round(
                Cov["covcorr"] / Cov["mean_frag_len"].mean()
            )
    ...
    def load_SNPs(self, f_snps):
        ...
        clust_choice = self.allelic_clusters["snps_to_clusters"][
            self.allelic_sample
        ]
        ...
        seg_bdy = np.r_[
            0, list(self.segmentations[self.allelic_sample].keys()), len(SNPs)
        ]
        ...
        for i, (st, en) in enumerate(seg_bdy):
            ...
            self.full_cov_df.loc[
                (self.full_cov_df["start_g"] >= st_g)
                & (self.full_cov_df["end_g"] <= en_g),
                "allelic_seg_overlap",
            ] = i
        ...
        if not self.wgs:
            ...
            AI = self.full_cov_df.groupby("allelic_seg_overlap").agg(
                {"start_g": min, "end_g": max}
            )
            AI.loc[-1, :] = np.nan
            ...
            unmap_idx = SNPs.index.isin(tidx_ext.index) & (
                SNPs["targ_idx"] == -1
            )
    ...
    def load_covariates(self, cov_df):
        ...
        cov_df = cov_df.loc[
            (cov_df.mean_frag_len > 0) & (cov_df.std_frag_len > 0)
        ...
        tidx = mut.map_mutations_to_targets(
            cov_df, F, inplace=False, poscol="midpoint"
        )
        F = (
            F.loc[tidx]
            .set_index(tidx.index)
            .iloc[:, 3:]
            .rename(columns=lambda x: "C_RT-" + x)
        )
        ...
        if self.f_faire is not None:
            ...
            F = (
                ...
                .set_index(tidx.index)
                .iloc[:, 3:]
                .rename(columns=lambda x: "C_FAIRE-" + x)
            )
            cov_df = pd.concat([cov_df, F], axis=1)
        ...
        if self.f_extracov_bed_list is not None:
            ...
            with open(self.f_extracov_bed_list, "r") as f:
                for i, bed_file in tqdm.tqdm(enumerate(f.readlines())):
                    ...
                    tidx = mut.map_mutations_to_targets(
                        cov_df, extracov_df, inplace=False, poscol="midpoint"
                    )
                    extracov_df = (
                        extracov_df.loc[tidx]
                        .set_index(tidx.index)
                        .iloc[:, 3:]
                        .rename(columns=lambda x: f"C_extracov-{i},{x - 3}")
                    )
    ...
    def filter_and_scale_covariates(self, cov_df):
        ...
        cov_df = pd.concat(
            [
                cov_df,
                Cslice.loc[:, lztcols]
                .apply(lambda x: zt(np.log(x + 1)))
                .rename(columns=lambda x: x + "_lz"),
            ],
            axis=1,
        )
        ...
        ## filter covariates
        Cslice = cov_df.loc[
            :, cov_df.columns.str.contains("(?:^C_.*z$|C_log_len)")
        ]
    ...
    def assign_clusters(self):
        ...
        for targ, D in tqdm.tqdm(
            ...
            if len(seg_idx) == 1 or (seg_idx[0] == seg_idx).all():
                # short circuit second condition for efficiency
        ...
        acmap = (
            Cov_overlap.loc[
                Cov_overlap["allelic_cluster"] != -1,
                ["seg_idx", "allelic_cluster"],
            ]
            .drop_duplicates()
            .set_index("seg_idx")
        )
        ...
        with pd.option_context(
            ...
            Cov_overlap.loc[:, "allelic_cluster"] = acmap.loc[
                Cov_overlap["seg_idx"]
            ].values
    ...
    def make_regressors(
        ...
        """
        Prepares the LNP regression.
        The LNP regression data

        Args:
            cov_df (pd.DataFrame): A dataframe containing the allelic segmentation and covariate data.

        Returns:

        A tuple containing:
        * The mapping matrix. This is a `[observations, clusters]` matrix with 0s and 1s
            mapping observations to clusters.
        * The observations. This is the `fragcorr` value for each bin.
        * The covariates. This is a `[observations, covariates]` matrix containing
            the covariates for each observation.
        * The filtered dataframe.
        ...
        # regressor
        observations = np.c_[cov_df["fragcorr"]]

        # intercept matrix (one intercept per allelic segment)
        Pi = np.zeros(
            [len(cov_df), cov_df["seg_idx"].max() + 1], dtype=np.float16
        )
        Pi[np.r_[0 : len(cov_df)], cov_df["seg_idx"]] = 1
        Pi = Pi[
            :, Pi.sum(0) > 0
        ]  # prune zero columns in Pi (allelic segments that got totally eliminated)

        # remove non z-transformed covar cols
        drop_cols = list(
            cov_df.columns[
                cov_df.columns.str.contains("^C_.*")
                & ~cov_df.columns.str.contains(
                    "^C_.*z$|^C_frag_len$|^C_log_len"
                )
            ]
        )
        ...
        return (
            ...
            C,
            pd.concat(
                [cov_df.loc[:, ~col_idx], cov_df.loc[:, sorted_covar_columns]],
                axis=1,
            ),
        )
...
def poisson_outlier_filter(
    ...
    pois_log_liks = stats.poisson(mu=residuals.mean()).logpmf(
        residuals.astype(int)
    )
...
def edge_outliers(cov_df, sigma=5):
    ...
    for idx, seg_df in list(cov_df.groupby("seg_idx")):
        ...
        seg_std = f_arr[2:].std()
        if (
            f_arr[0] < seg_mean - sigma * seg_std
            or f_arr[0] > seg_mean + sigma * seg_std
        ):
            # check if second bin is also outlier
            if not (
                f_arr[1] < seg_mean - sigma * seg_std
                or f_arr[1] > seg_mean + sigma * seg_std
            ):
        ...
        if (
            ...
            if not (
                f_arr[-2] < seg_mean - sigma * seg_std
                or f_arr[-2] > seg_mean + sigma * seg_std
            ):
...
def aggregate_clusters(
    ...
    if coverage_dir is None and f_file_list is None:
        raise ValueError(
            "need to pass in either coverage_dir or file_list txt file!"
        )
    if coverage_dir is not None and f_file_list is not None:
        raise ValueError(
            "need to pass in either coverage_dir or file_list txt file!, got both!"
        )

    # get results files from the directory provided or from the file list provided
    if coverage_dir is not None:
        seg_files = nat_sort(
            glob.glob(os.path.join(coverage_dir, "cov_mcmc_data_allelic_seg*"))
        )
    ...
    for d in range(num_draws):
        ...
        for seg in seg_idxs:
            ...
            coverage_segmentation[seg_indices, d] = (
                seg_results[seg][:, d] + global_counter
            )
    ...
    Pi[range(len(MAP_seg)), MAP_seg.astype(int)] = 1.0
    ## generate covars
    covar_columns = sorted(
        cov_df.columns[cov_df.columns.str.contains("(?:^C_.*_l?z$|C_log_len)")]
    )
