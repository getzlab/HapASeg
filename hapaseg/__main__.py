...
from .run_coverage_MCMC import (
    CoverageMCMCRunner,
    aggregate_clusters,
    aggregate_burnin_files,
)
from .coverage_DP import Coverage_DP
from .a_cov_DP import (
    generate_acdp_df,
    AllelicCoverage_DP,
    AllelicCoverage_DP_runner,
)


def parse_args():
    parser = argparse.ArgumentParser(
        description="Call somatic copynumber alterations taking advantage of SNP phasing"
    )

    parser.add_argument("--output_dir", default=".")
    parser.add_argument(
        "--capy_ref_path",
        default="/home/opriebe/data/ref/hg19/Homo_sapiens_assembly19.fasta",
    )
    subparsers = parser.add_subparsers(dest="command")

    ## run
    standalone = subparsers.add_parser(
        "run", help="Run HapASeg as a standalone module"
    )
    standalone.add_argument(
        "--n_workers", default=multiprocessing.cpu_count() - 1
    )
    ...
    phasing_ref_group.add_argument(
        "--bam", help="BAM to use for read-backed phasing"
    )
    phasing_ref_group.add_argument(
        "--bai", help="BAI to use for read-backed phasing"
    )
    ...
    ## amcmc
    amcmc = subparsers.add_parser(
        "amcmc", help="Run allelic MCMC on a range of SNPs"
    )

    input_group = amcmc.add_mutually_exclusive_group(required=True)
    ...
    concat = subparsers.add_parser(
        "concat", help="Concatenate burned-in chunks"
    )
    ...
    concat_arms.add_argument(
        "--arm_results",
        help="file containing paths to arm mcmc objects",
        required=True,
    )
    ## DP
    dp = subparsers.add_parser(
        "dp", help="Run DP clustering on allelic imbalance segments"
    )
    dp.add_argument("--seg_dataframe", required=True)
    dp.add_argument(
        "--ref_fasta", required=True
    )  # TODO: only useful for chrpos->gpos; will be removed when this is passed from load
    dp.add_argument("--cytoband_file", required=True)
    dp.add_argument("--wgs", action="store_true", default=False)

    ## coverage MCMC
    coverage_mcmc = subparsers.add_parser(
        "coverage_mcmc",
        help="Run TCR segmentation on all allelic imbalance clusters",
    )
    coverage_mcmc.add_argument(
        "--coverage_csv",
    ...
    coverage_mcmc.add_argument(
        "--SNPs_pickle", help="pickled dataframe containing SNPs"
    )
    coverage_mcmc.add_argument(
        "--segmentations_pickle",
        help="pickled sorteddict containing allelic imbalance segment boundaries",
        required=True,
    )
    coverage_mcmc.add_argument(
        "--covariate_dir",
    ...
    coverage_mcmc.add_argument(
        "--num_draws",
        type=int,
        help="number of draws to take from coverage segmentation MCMC",
        default=50,
    )
    coverage_mcmc.add_argument(
        "--allelic_sample",
    ...
    # collect ADP data
    collect_adp = subparsers.add_parser(
        "collect_adp", help="collect ADP resuts from shards"
    )
    collect_adp.add_argument(
        "--dp_results", help="path to txt file with paths to dp results"
    )

    ## proprocessing coverage MCMC data for scatter tasks
    preprocess_coverage_mcmc = subparsers.add_parser(
        "coverage_mcmc_preprocess",
        help="Preform preprocessing on ADP results to allow for coverage mcmc scatter jobs",
    )
    preprocess_coverage_mcmc.add_argument(
        "--coverage_csv",
        help="csv file containing '['chr', 'start', 'end', 'covcorr', 'covraw'] data",
        required=True,
    )
    ...
    preprocess_coverage_mcmc.add_argument(
        "--SNPs_pickle", help="pickled dataframe containing SNPs", required=True
    )
    preprocess_coverage_mcmc.add_argument(
        "--segmentations_pickle",
        help="pickled sorteddict containing allelic imbalance segment boundaries",
    ...
    preprocess_coverage_mcmc.add_argument(
        "--repl_pickle",
        help="pickled dataframe containing replication timing data",
        required=True,
    )
    preprocess_coverage_mcmc.add_argument(
        "--faire_pickle",
        help="pickled dataframe containing FAIRE data",
        required=False,
    )
    preprocess_coverage_mcmc.add_argument(
        "--normal_coverage_csv",
        help="csv file in the format of the tumor coverage file, but for the normal",
        required=False,
    )
    preprocess_coverage_mcmc.add_argument(
        "--extra_covariates_bed_paths",
        help="path to newline delimited file listing multiple BED files to use as additional covariates",
        required=False,
    )
    preprocess_coverage_mcmc.add_argument(
        "--gc_pickle",
        help="pickled dataframe containing precomputed gc content. This is not required but will speed up runtime if passed",
        default=None,
    )
    preprocess_coverage_mcmc.add_argument(
        "--allelic_sample",
    ...
    preprocess_coverage_mcmc.add_argument(
        "--ref_fasta", help="reference fasta file", required=True
    )
    preprocess_coverage_mcmc.add_argument(
        "--bin_width",
        help="Coverage bin width (for WGS only)",
        default=1,
        type=int,
    )
    preprocess_coverage_mcmc.add_argument(
        "--wgs",
        help="If not WGS, expand targets by +-150b to capture more SNPs",
        action="store_true",
    )

    ## running coverage mcmc on single cluster for scatter task
    coverage_mcmc_shard = subparsers.add_parser(
        "coverage_mcmc_shard", help="run coverage mcmc on single ADP cluster"
    )
    coverage_mcmc_shard.add_argument(
        "--preprocess_data",
        help="path to numpy object containing preprocessed data: covariate matrix (C), global beta, ADP cluster mu's, covbin ADP cluster assignments (all_mu), covbin raw coverage values (r)",
        required=True,
    )
    coverage_mcmc_shard.add_argument(
        "--allelic_seg_indices",
        help="path to pickled pandas dataframe containing coverage bin indices for each alleic segment",
        required=True,
    )
    coverage_mcmc_shard.add_argument(
        "--allelic_seg_idx",
        help="which allelic segment to perform coverage segmentation on.",
        required=True,
        type=int,
    )
    coverage_mcmc_shard.add_argument(
    ...
    coverage_mcmc_shard.add_argument(
        ...
        type=int,
        default=1,
        help="size of uniform bins if using. Otherwise 1.",
    )
    coverage_mcmc_shard.add_argument(
        "--burnin_files",
        type=str,
        help="txt file containing burnt in segment assignments",
    )

    ## collect coverage MCMC shards
    collect_cov_mcmc = subparsers.add_parser(
        "collect_cov_mcmc", help="collect sharded cov mcmc results"
    )
    collect_cov_mcmc.add_argument(
        "--coverage_dir",
        help="path to the directory containing the coverage mcmc results",
    )
    collect_cov_mcmc.add_argument(
        "--cov_mcmc_files",
    ...
    collect_cov_mcmc.add_argument(
        "--seg_indices_pickle",
        help="path to segment indices dataframe pickle",
        required=True,
    )
    collect_cov_mcmc.add_argument(
        "--bin_width",
    ...
    ## Coverage DP
    coverage_dp = subparsers.add_parser(
        "coverage_dp", help="Run DP clustering on coverage segmentations"
    )
    coverage_dp.add_argument(
        "--f_cov_df", help="path to saved filtered coverage dataframe"
    )
    coverage_dp.add_argument(
        "--cov_mcmc_data",
    ...
    coverage_dp.add_argument(
        "--num_segmentation_samples",
        type=int,
        help="number of segmentation samples to use",
    )
    coverage_dp.add_argument(
        "--sample_idx",
        type=int,
        help="index of segmentation draw to use if scattering over segmentation draws",
    )
    coverage_dp.add_argument(
        "--num_dp_samples",
        type=int,
    ...
    # generate df
    gen_acdp_df = subparsers.add_parser(
        "generate_acdp_df", help="generate dataframe for acdp clustering"
    )

    gen_acdp_df.add_argument(
        "--snp_dataframe", help="path to dataframe containing snps"
    )
    gen_acdp_df.add_argument(
        "--cdp_object", help="path to coverage DP output object"
    )
    gen_acdp_df.add_argument(
        "--cdp_filepaths", help="paths of scattered DP objects"
    )
    gen_acdp_df.add_argument(
        "--cov_df_pickle",
        help="path to cov_df pickle. For running on cov segmentation results directly",
    )
    gen_acdp_df.add_argument(
        "--cov_seg_data",
        help="path to collected cov mcmc segmentation data. For running on cov segmentation results directly",
    )
    gen_acdp_df.add_argument(
        "--allelic_clusters_object",
        help="npy file containing allelic dp segs-to-clusters results",
    ...
    gen_acdp_df.add_argument(
        "--allelic_draw_index",
        help="index of ADP draw used for coverage MCMC",
        type=int,
        default=-1,
    )
    gen_acdp_df.add_argument(
    ...
    gen_acdp_df.add_argument(
        ...
        action="store_true",
        default=False,
        help="wgs mode creates acdp df for only the best draw",
    )

    # run acdp clustering
    ac_dp = subparsers.add_parser(
        "allelic_coverage_dp",
        help="Run DP clustering on allelic coverage tuples",
    )
    ac_dp.add_argument(
        "--cov_seg_data", help="path to savez file containing global beta"
    )
    ac_dp.add_argument("--acdp_df_path", help="path to acdp dataframe")
    ac_dp.add_argument(
        "--num_samples", type=int, help="number of samples to take"
    )
    ...
    ac_dp.add_argument(
        "--wgs",
        help="flag to determine if sample is whole genome",
        default=False,
        action="store_true",
    )
    ac_dp.add_argument(
        "--lnp_data_pickle", help="path to lnp data dictionary", required=True
    )
    ac_dp.add_argument(
        "--use_single_draw",
        help="flag to force acdp to only use best draw",
        default=False,
        action="store_true",
    )
    ac_dp.add_argument(
        ...
        type=bool,
        default=True,
        help="run clustering with warmstart",
    )

    # final summary plot
    fp = subparsers.add_parser(
        "summary_plot", help="summary plot of HapASeg results"
    )
    fp.add_argument(
        "--snps_pickle", required=True, help="path to all SNPs pickle file"
    )
    fp.add_argument(
        "--adp_results",
        required=True,
        help="path to the allelic DP results file",
    )
    fp.add_argument(
        "--segmentations_pickle",
        required=True,
        help="path to final ADP segmentations",
    )
    fp.add_argument(
        "--acdp_model", required=True, help="path to acdp model pickle"
    )
    fp.add_argument(
        "--ref_fasta",
        required=True,
        help="path to matching reference fasta file",
    )
    fp.add_argument(
        "--cytoband_file", required=True, help="path to cytoband file"
    )
    fp.add_argument(
        "--hapaseg_segfile",
        required=True,
        help="path to hapaseg format segfile (can be clustered or unclustered)",
    )
    fp.add_argument(
        "--outdir", required=False, default="./", help="path to save figure"
    )
...
def main():
    ...
    if args.command == "load_snps":
        ...
        t = mut.map_mutations_to_targets(
            snps.allele_counts, chromosome_intervals, inplace=False
        )
        ...
        groups = groups.loc[
            groups["bdy"].apply(np.diff) > 5
        ]  # XXX: is 5 SNPs too small?

        # chunk arms
        groups["ranges"] = groups["bdy"].apply(
            lambda x: np.r_[x[0] : x[1] : args.chunk_size, x[1]]
        )
        ...
        short_last_chunks = groups.loc[
            ...
            & (
                groups["ranges"].apply(lambda x: x[-1] - x[-2])
                < args.chunk_size * 0.05
            ),
            "ranges",
        ]
        groups.loc[short_last_chunks.index, "ranges"] = short_last_chunks.apply(
            lambda x: np.r_[x[:-2], x[-1]]
        )
        ...
        try:
            H.visualize(
                show_CIs=args.betahyp != -1
            )  # only plot SNP CI's if there are few SNPs (not WGS)
        ...
        if len(intervals) != len(args.chunks):
            raise ValueError(
                "Length mismatch in supplied chunks and interval file!"
            )
        ...
        a = (
            X.loc[(slice(None), 0), "ALT_COUNT"].droplevel(1)
            + X.loc[(slice(None), 1), "REF_COUNT"].droplevel(1)
            + 1
        )
        b = (
            X.loc[(slice(None), 0), "REF_COUNT"].droplevel(1)
            + X.loc[(slice(None), 1), "ALT_COUNT"].droplevel(1)
            + 1
        )
        ...
        for opt_iter in range(3):
            ...
            n_beta_samp = np.r_[
                10, 100, np.maximum(100, int(1e8 / tot_SNPs.sum()))
            ][opt_iter]
            ...
            if opt_iter > 0:
                refbias_dom = np.linspace(
                    *refbias_dom[
                        np.minimum(
                            np.argmin(refbias_dif) + np.r_[-2, 2],
                            len(refbias_dom) - 1,
                        )
                    ],  # search +- 2 grid points of previous optimum; clip to rb = 1
            ...
            for j, rb in pbar:
                pbar.set_description(
                    f"[{refbias_dom.min():0.2f}:{refbias_dom.max():0.2f}:{len(refbias_dom)}] {refbias_dom[refbias_dif.argmin()]:0.4f} ({n_beta_samp} MC samples)"
                )
                ...
                refbias_dif[j] = (
                    absdif @ tot_SNPs[use_idx] / tot_SNPs[use_idx].sum()
                )
        ...
        for arm, Ra in R.groupby("arm"):
            ...
            A.marg_lik = np.full(
                A.n_iter, np.nan
            )  # n_iter and size of this array will be reset later
        ...
        n_samps = int(
            np.minimum(
                np.inf,
                A.loc[~A["results"].isna(), "results"].apply(
                    lambda x: len(x.breakpoint_list)
                ),
            ).min()
        )
        ...
        np.savez(
            output_dir + "/allelic_DP_SNP_clusts_and_phase_assignments.npz",
    ...
    ## running coverage mcmc on all clusters
        ...
        cov_mcmc_runner = CoverageMCMCRunner(
            args.coverage_csv,
        ...
        np.savez(
            os.path.join(output_dir, "cov_mcmc_data"),
            seg_samples=seg_samples,
            beta=beta,
            mu_i_samples=mu_i_samples,
        )
    ...
    ## preprocess ADP data to run scattered coverage mcmc jobs on each ADP cluster
        ...
        seg_g = cov_df.groupby(
            "seg_idx"
        )  # NOTE: seg_idx may not be contiguous if any allelic segments were dropped
        ...
        seg_g_idx.to_pickle(
            os.path.join(output_dir, "allelic_seg_groups.pickle")
        )
    ...
    ## run scattered coverage mcmc job using preprocessed data
        ...
        # collect the results
        segment_samples, global_beta, mu_i_samples, ll_samples = (
        ...
        np.savez(
            os.path.join(output_dir, data_save_str),
            seg_samples=segment_samples,
            beta=global_beta,
            mu_i_samples=mu_i_samples,
        ...
        if args.coverage_dir:
            ...
            if args.cov_df_pickle is None:
                raise ValueError(
                    "cov_df_pickle argument required for passing shard file"
                )
            full_segmentation, mu, beta, ll_samples = aggregate_clusters(
                seg_indices_pickle=args.seg_indices_pickle,
                f_file_list=args.cov_mcmc_files,
            ...
            raise ValueError(
                "must pass in either a directory or a txt file listing mcmc results"
            )

        ## save these results to new aggregated file
        if args.coverage_dir:
            np.savez(
                os.path.join(args.coverage_dir, "cov_mcmc_collected_data"),
                seg_samples=full_segmentation,
                beta=beta,
                ll_samples=ll_samples,
            )
        else:
            np.savez(
                os.path.join(output_dir, "cov_mcmc_collected_data"),
                seg_samples=full_segmentation,
                beta=beta,
                ll_samples=ll_samples,
            )
        ...
        covar_columns = sorted(
            cov_df.columns[
                cov_df.columns.str.contains("(?:^C_.*_l?z$|C_log_len)")
            ]
        )
        emu = np.exp(mu)
        ...
        plt.scatter(
            ...
            marker=".",
            s=1,
            c=np.array(["dodgerblue", "orangered"])[seg_idx % 2],
            alpha=0.5,
        )
        plt.scatter(
            cov_df["start_g"], emu[seg_idx].ravel(), marker=".", s=1, c="k"
        )
        ...
        cov_dp_runner = Coverage_DP(
            segmentation_samples, beta, cov_df, args.bin_width
        )
        # print(args.sample_idx, args.num_segmentation_samples, args.num_dp_samples, flush=True)
        if args.sample_idx is not None:
            cov_dp_runner.run_dp(
                1, args.num_dp_samples, sample_idx=args.sample_idx
            )
            model_save_path = "Cov_DP_model_{}.pickle".format(args.sample_idx)
            viz_sample = 0
        else:
            cov_dp_runner.run_dp(
                args.num_segmentation_samples, args.num_dp_samples
            )
        ...
        # save visualization
        cov_dp_runner.visualize_DP_run(
            viz_sample,
            os.path.join(
                output_dir,
                "cov_dp_visual_draw_{}".format(args.num_dp_samples - 1),
            ),
        )
        ...
        if args.cdp_object is not None:
            ...
            acdp_df, lnp_data, opt_cdp_idx = generate_acdp_df(
                args.snp_dataframe,
                args.allelic_clusters_object,
        ...
        # for running directly from cov_mcmc segments
            ...
            raise ValueError(
                "must pass a cdp filepath, list of cdp filepaths or cov_df pickle and mcmc seg file"
            )
        ...
        acdp = AllelicCoverage_DP_runner(
            acdp_df,
            beta,
            args.cytoband_file,
            lnp_data,
        ...
        adp_obj = load_DP_object_from_outputs(
            args.snps_pickle, args.adp_results, args.segmentations_pickle
        )
        ...
        for i in cov_df.segment_ID.unique():
            ...
            axs[1].plot(
                (start, end), (mu_exp, mu_exp), linewidth=0.5, color="k"
            )
            axs[1].scatter(
                (start + end) / 2, mu_exp, marker=".", s=1, color="k"
            )
        plt.sca(axs[1])
        plt.yticks(fontsize=6)
        hs_utils.plot_chrbdy(args.cytoband_file)
        ...
        seg_df["start_g"] = seq.chrpos2gpos(
            seg_df["Chromosome"], seg_df["Start.bp"], ref=args.ref_fasta
        )
        seg_df["end_g"] = seq.chrpos2gpos(
            seg_df["Chromosome"], seg_df["End.bp"], ref=args.ref_fasta
        )
        for i, row in seg_df.iterrows():
            seg_len = row["End.bp"] - row["Start.bp"]
            mu_major = mpl.patches.Rectangle(
                (row["start_g"], row["mu.major"] - 2),
                seg_len,
                4,
                alpha=0.5,
                color="r",
            ...
            mu_minor = mpl.patches.Rectangle(
                (row["start_g"], row["mu.minor"] - 2),
                seg_len,
                4,
                alpha=0.5,
                color="b",
