...
mutect = wolf.ImportTask(
    task_path="git@github.com:getzlab/MuTect1_TOOL.git", task_name="M1"
)
...
class HapASeg_Preprocess_Callstats(wolf.Task):
    inputs = {"callstats": None, "sample_name": None, "dummy_normal": False}
    ...
    output_patterns = {
        "hapaseg_hetsite_depths": "*depth.tsv",
...
class ASCAT_Preprocess_Callstats(wolf.Task):
    inputs = {"callstats": None, "sample_name": None, "ascat_loci_list": None}
    script = """
    preprocess_raw_data.py --sample_name ${sample_name}\
...
def Generate_Hapaseg_Raw_Data_Workflow(
    ...
    if ref_genome_build == "hg38":
        ...
        raise ValueError(
            f"did not recognize ref build {ref_genome_build}. Expected hg38 or hg19"
        )
    ...
    subset_intervals = interval_gather(
        split_intervals_task["interval_files"], primary_contigs
    )
    ...
    tumor_cov_gather_task = wolf.Task(
        ...
        inputs={
            "coverage_beds": [tumor_cov_collect_task["coverage"]],
    ...
    m1_gather = wolf.Task(
        name="m1_gather",
        inputs={
            "callstats_array": [m1_scatter["mutect1_cs"]],
            "sample_name": sample_name,
        },
        script="""
    ...
    if chrs_to_exclude != "":
        ...
        chr_exclusion_task = wolf.Task(
            name="exclude_chrs",
            inputs={
                "mutect_cs": m1_gather["cs_gather"],
            ...
            outputs={"mutect_cs": "*.tsv", "covcollect_bed": "*.bed"},
        )
    ...
    if preprocess:
        ...
        hapaseg_cs_task = HapASeg_Preprocess_Callstats(
            inputs={
        ...
        if not dummy_normal:
            ...
            convert_task = wolf.Task(
                ...
                inputs={
                    ...
                    "sample_name": "HAPASEG_CONVERT_HET_PULLDOWN_TASK",
            ...
            combine_vcf_task = wolf.Task(
                name="combine_vcfs",
                inputs={
                    "vcf_array": [eagle_task["phased_vcf"]],
                    "sample_name": sample_name,
                },
        ...
        facets_cs_task = Facets_Preprocess_Callstats(
            inputs={
                "callstats": mutect_callstats_output,
                "sample_name": sample_name,
        ...
        ascat_cs_task = ASCAT_Preprocess_Callstats(
            inputs={
                "callstats": mutect_callstats_output,
                "sample_name": sample_name,
    ...
    if upload_bucket_gs_path is not None:
        ...
        upload_root_task = UploadToBucket(
            files=[mutect_callstats_output, covcollect_output],
            bucket=upload_bucket_gs_path,
        )

        if preprocess:
            if not dummy_normal:
                upload_phasing_task = UploadToBucket(
                    files=combine_vcf_task["combined_vcf"],
                    bucket=upload_bucket_gs_path,
                )

            upload_hapaseg_task = UploadToBucket(
                files=[
                    hapaseg_cs_task["hapaseg_hetsite_depths"],
                    hapaseg_cs_task["hapaseg_filtered_cs"],
                ],
                bucket=upload_bucket_gs_path.rstrip("/") + "/hapaseg/",
            )

            upload_facets_task = UploadToBucket(
                files=[
                    facets_cs_task["facets_variant_depths"],
                    facets_cs_task["facets_filtered_variants"],
                ],
                bucket=upload_bucket_gs_path.rstrip("/") + "/facets/",
            )

            upload_ascat_task = UploadToBucket(
                files=[
                    ascat_cs_task["ascat_variant_depths"],
