...
def parse_args():
    ...
    hatchet_gen.add_argument("--total_reads_dir", required=True, help="path to dir contianing <contig>.total.gz files generated by count-reads")
    hatchet_gen.add_argument("--thresholds_snps_dir", required=True, help="path to dir containing <contig>.thresholds.gz files generated by count-reads")
...
def main():
    ...
    if args.command == "hapaseg":
        ...
        generate_hatchet_files(sim_profile_pickle=args.sim_profile,
                        ...
                        total_reads_dir=args.total_reads_dir,
                        thresholds_snps_dir=args.thresholds_snps_dir,
                        out_dir=output_dir,
                        out_label=args.out_label,
                        parallel = args.parallel)
...
def generate_hatchet_files(sim_profile_pickle=None,
                           ...
                           normal_vcf_path=None,
                           tumor_baf_path=None,
                           total_reads_dir=None, # path to directory containing total reads files
                           thresholds_snps_dir=None, # path to directory containing total reads files
                           out_dir = "./",
                           out_label=None,
                           parallel=False):
    ...
    normal_sim_label = out_label + '_normal_sim'
    ...
    total_reads_paths = glob.glob(os.path.join(total_reads_dir, 'chr*.total.gz'))
    thresholds_snps_paths = glob.glob(os.path.join(total_reads_dir, 'chr*.thresholds.gz'))
    ...
    for total_fn, threshold_fn in zip(sorted(total_reads_paths), sorted(thresholds_snps_paths)):
        ...
        shutil.copyfile(threshold_fn, os.path.join(out_dir, "hatchet_data", os.path.basename(threshold_fn)))

        int_corr = pd.concat([this_thresholds_df['threshold'], pd.Series(np.append(this_thresholds_df.loc[1:, 'threshold'], this_thresholds_df.values[-1])), this_read_df['n_int_reads']], axis=1)
        int_corr.columns = ['start', 'end', 'covcorr']
        chrom = os.path.basename(total_fn).rstrip('total.gz')
        int_corr['contig'] = chrom
        pos_corr = pd.concat([this_thresholds_df['threshold'], this_thresholds_df['threshold'] + 1, this_read_df['n_pos_reads']], axis=1)
    ...
    for total_fn in total_fn_list_contigs:
        read_combined_df[read_combined_df['contig'] == total_fn.rstrip('total.gz')].drop(columns='contig').to_csv(os.path.join(out_dir, f'hatchet_data/{total_fn}'), sep='\t', header=False, index=False, compression='gzip')
