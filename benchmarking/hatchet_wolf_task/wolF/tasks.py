...
class HATCHET_count_alleles(wolf.Task):
    ...
    def script(self):
        ...
        hatchet count-alleles --normal ./normal.bam --tumors ./tumor.bam --reference ${ref_fasta}\
    ...
    resources = { "mem": "2G"}
    docker = "gcr.io/broad-getzlab-workflows/hatchet:v1"
...
class HATCHET_count_reads(wolf.Task):
    ...
    def script(self):
        ...
        ln -s ${normal_bam} ./normal.bam; ln -s ${normal_bai} ./normal.bam.bai
        ln -s ${tumor_bam} ./tumor.bam; ln -s ${tumor_bai} ./tumor.bam.bai
        hatchet count-reads --normal ./normal.bam --tumor ./tumor.bam\
         --refversion ${reference_genome_version}\
        --baffile ${normal_baf} --outdir .\
        --readquality ${read_quality} --processes ${num_processes}"""

        if self.conf["inputs"]["chromosomes"]:
            script += " --chromosomes ${chromosomes}"
        
        # need to flip sample names since the function makes sure that the snps in the vcf match 
        # the tumor name
        if self.conf["inputs"]["sample_names"]:
            flipped_names = ' '.join(reversed(self.conf["inputs"]["sample_names"].split(" ")))
            script += " --samples " + flipped_names
...
class HATCHET_phase_snps(wolf.Task):
    ...
    resources = {"cpus-per-task": 8, "mem": "32G"}
    docker = "gcr.io/broad-getzlab-workflows/hatchet:v0"
