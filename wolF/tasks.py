...
class hapaseg(wolf.Task):
    ...
    docker = "gcr.io/broad-getzlab-workflows/hapaseg:beta_v1328"
...
class hapaseg_prepare_coverage_mcmc(wolf.Task):
    ...
    docker = "gcr.io/broad-getzlab-workflows/hapaseg:beta_v1331"
    resources = {"mem": "6G"}
...
class hapaseg_acdp_generate_df(wolf.Task):
    ...
    docker = "gcr.io/broad-getzlab-workflows/hapaseg:beta_v1331"
    resources = {"cpus-per-task": 2, "mem": "5G"}
...
class hapaseg_run_acdp(wolf.Task):
    ...
    def script(self):
        ...
        if self.conf["inputs"]["wgs"]:
            script += " --wgs"
        if self.conf["inputs"]["use_single_draw"]:
