...
def samtools_view(bam: Path) -> List[List[str]]:
    proc = subprocess.run(["samtools", "view", "-H", str(bam)], capture_output=True, text=True, check=True)
    return [row.split("\t") for row in proc.stdout.split("\n")]
        ...
        for idx in pending_processes[:]:  # Create a copy to safely modify during iteration
            ...
            if return_code is not None:
                ...
                if return_code != 0:
                    ...
                    raise ProcessFailedError(process_id=process.pid, return_code=return_code, stderr_file=stderr_file)
...
def run_parallel_tasks(
    out_path: Path, task_name: str, script_generator: Callable, script_args: Dict[str, Any], max_jobs: int = 8
):
    """
    ...
    if not list_args:
        raise ValueError(
            "No list arguments found in script_args. At least one argument must be a list for chunked execution."
        )
    ...
    if chunks_to_run:
        print(
            f"Running {len(chunks_to_run)} of {num_chunks} {task_name} tasks in parallel (skipping {num_chunks - len(chunks_to_run)} completed tasks)..."
        )
        scripts_to_run = {name: all_scripts[name] for name in chunks_to_run}
        run_gnu_parallel_tasks_backend(
            scripts_to_run, parent_out_path.joinpath(f"{task_name}_runlog"), max_jobs=max_jobs
        )
    else:
...
def make_mutect_script(
    ...
    output_dicts = {"mutect1_cs": cs_path, "mutect1_vcf": vcf_path}

    return script, output_dicts


# Mutect1
def make_mutect_scripts(
    out_dir: Path,
    n_bam: Path,
    n_bai: Path,
    t_bam: Path,
    ...
    if n_tasks is None:
        n_tasks = (os.cpu_count() or 8) * 8

    # Binary searching for a split length that fits the number of tasks.
    low = max(contigs.values()) // n_tasks
    high = max(contigs.values())

    while low + 1 < high:
        mid = (low + high) // 2
        task_count = sum(int(math.ceil(contig_length / mid)) for contig_length in contigs.values())
    ...
    for idx, intervals in enumerate(proc_intervals):
        ...
        if exclude_chimeric:
            script += " --exclude_chimeric_reads"
        if Path(intervals).exists():
            ln_intv_path = out_dir.joinpath(Path(intervals).stem + ".picard")
            script = f"ln -s {intervals} {ln_intv_path} && " + script
            script += f" -L {ln_intv_path}"
        else:
            script += f" -L {intervals}"

        scripts.append(script)

    output_dicts = {"mutect1_cs": cs_paths, "mutect1_vcf": vcf_paths}

    return scripts, output_dicts


# Het pulldown
def make_het_pulldown_script(
    out_path: Path,
    callstats_file=None,
    ref_fasta=None,
    ref_fasta_idx=None,
    ...
    script = (
        f"hetpull.py -g -c {callstats_file} -r {ref_fasta} -o {out_stem} "
...
def convert_hetpull_2_vcf_script(
    out_path: Path, genotype_file=None, ref_fasta=None, ref_fasta_idx=None, ref_fasta_dict=None, sample_name="test"
):
    out_path.mkdir(exist_ok=True, parents=True)
    script = f"""
    set -x
    ...
    outputs = {
        "bcfs": [out_path.joinpath(f"chr{c}.chrsplit.bcf") for c in range(1, 23)]
        + [out_path.joinpath("chrX.chrsplit.bcf")],
        "bcf_idxs": [out_path.joinpath(f"chr{c}.chrsplit.bcf.csi") for c in range(1, 23)]
        + [out_path.joinpath("chrX.chrsplit.bcf.csi")],
    }
    return script, outputs


# Eagle
def make_eagle_script(
    out_path: Path,
...
def combine_eagle_vcfs_script(out_path: Path, vcf_paths):
    out_path.mkdir(exist_ok=True, parents=True)
