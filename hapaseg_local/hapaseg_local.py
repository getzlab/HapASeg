...
def hapaseg_local_main(
    ...
    covcollect_n_script, covcollect_n_results_dict = make_covcollect_script(
        covcollct_normal_out_path, normal_bam, normal_bai, WGS_bin_size if wgs else WES_target_list, single_ended
    )
    covcollect_t_script, covcollect_t_results_dict = make_covcollect_script(
        covcollct_tumor_out_path, tumor_bam, tumor_bai, WGS_bin_size if wgs else WES_target_list, single_ended
    )

    mutect_scripts, mutect_results_dict = make_mutect_scripts(
        out_dir=mutect_out_path,
        n_bam=normal_bam,
        n_bai=normal_bai,
        t_bam=tumor_bam,
        t_bai=tumor_bai,
    ...
    if scripts_to_run:
        print(f"Running {len(scripts_to_run)} tasks.")
        run_gnu_parallel_tasks_backend(
            scripts_to_run, out_path.joinpath("cov_mutect_runlog"), max_jobs=job_resources(4, 4, max_cpus, max_memory)
        )
    else:
    ...
    convert_hp_results_dict = run_single_task(
        out_path,
        "Het_2_vcfs",
        convert_hetpull_2_vcf_script,
        dict(
            genotype_file=hetpull_results_dict["normal_genotype"],
            ref_fasta=ref_config["ref_fasta"],
            ref_fasta_idx=ref_config["ref_fasta_idx"],
            ref_fasta_dict=ref_config["ref_fasta_dict"],
        ),
    ...
    def order_indices(ref_config, convert_hp_results_dict):
        ...
        R = R.set_index(
            R["ref_bcf_path"].apply(lambda x: re.search(r"^ALL.((?:chr)?(?:[^.]+)).*", os.path.basename(x)).groups()[0])
        )
        ...
        R2 = R2.set_index(
            R2["ref_bcf_idx_path"].apply(
                lambda x: re.search(r"^ALL.((?:chr)?(?:[^.]+)).*", os.path.basename(x)).groups()[0]
            )
        )
        ...
        F = F.set_index(
            F["bcf_path"].apply(lambda x: re.search(r"^((?:chr)?(?:[^.]+)).*", os.path.basename(x)).groups()[0])
        )

        # indices
        F2 = pd.DataFrame(dict(bcf_idx_path=convert_hp_results_dict["bcf_idxs"]))
        F2 = F2.set_index(
            F2["bcf_idx_path"].apply(lambda x: re.search(r"^((?:chr)?(?:[^.]+)).*", os.path.basename(x)).groups()[0])
        )
    ...
    comb_vcfs_results_dict = run_single_task(
        ...
        script_args={"vcf_paths": eagle_outputs["phased_vcf"]},
    ...
    for arm_file in hapaseg_amcmc_results_dict["arm_level_MCMC"]:
        with open(arm_file, "rb") as f:
            ...
            A.append(
                pd.Series(
                    {"chr": H.P["chr"].iloc[0], "start": H.P["pos"].iloc[0], "end": H.P["pos"].iloc[-1], "results": H}
                )
            )
    ...
    hapaseg_prepare_coverage_mcmc_results_dict = run_single_task(
        ...
        script_args={
            ...
            else (ref_config["cfdna_wes_faire_file"] if (is_cfdna and not wgs) else ref_config["faire_file"]),
            "gc_pickle": "",
    ...
    output_dict = {
        "covcollect_n_results_dict": covcollect_n_results_dict,
...
def save_final_results(output_dict, results_path):
    ...
    if (
        "hapaseg_summary_plot_results_dict" in output_dict
        and "hapaseg_summary_plot" in output_dict["hapaseg_summary_plot_results_dict"]
    ):
...
@click.option("--run-cdp", is_flag=True, default=False, help="Option to run coverage DP on WES data.")
@click.option(
    "--is-ffpe",
    is_flag=True,
    default=False,
...
@click.option("--max-cpus", default=8, type=int, help="Maximum number of CPUs to use. Default: 8")
@click.option("--max-memory", default=16, type=int, help="Maximum memory to use in GBs. Default: 16GB")
def run_hapaseg_local(out_dir, sample_name, **kwargs):
    """
